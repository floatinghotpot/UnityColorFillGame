...
{	
    ...
    public static Rect RectOperation = new Rect();
	...
	/// The show Skill zone.
	/// </summary>
	public bool showSkillZone = true;
	/// <summary>
	/// The Skill texture.
	/// </summary>
	...
	/// The show Skill zone.
	/// </summary>
	public bool showOutTexture = true;
	/// <summary>
	/// The Skill texture.
	/// </summary>
	...
	/// The Skill texture.
	/// </summary>
	private Vector2 joystickPostion;
	/// <summary>
	/// The Skill texture.
	/// </summary>
	public Rect outTouchArea;

	private bool bOutTouch = false;
	#endregion
		...
		if (Application.isPlaying)
		{
			...
			if (isActivated)
			{
				...
				if (oldJoystickAxis == Vector2.zero && joystickAxis != Vector2.zero)
				{
					if (interaction != InteractionType.Direct && interaction != InteractionType.Include)
					{
						CreateEvent(MessageName.On_JoystickMoveStart, joystickPostion);	
					}
				}				
				...
				if (joystickAxis != Vector2.zero)
				{
					...
					switch(interaction)
					{
						...
						case InteractionType.EventNotification:
							CreateEvent(MessageName.On_JoystickMove, joystickPostion);
							break;
				...
				else
				{
					if (!sendEnd)
					{
						CreateEvent(MessageName.On_JoystickMoveEnd, Vector2.zero);
						sendEnd = true;
					}
	...
	void OnGUI()
	{
		if (enable && visible && virtualJoystick)
		{
			GUI.depth = guiDepth;
			...
			// Out Texture;
			if(showOutTexture && outTexture != null)
			{
				// 绘制外圈范围;
				outTouchArea = new Rect(areaRect.x - zoneRadius, areaRect.y - zoneRadius, zoneRadius * 2, zoneRadius * 2);
				//GUI.DrawTexture(outTouchArea, outTexture, ScaleMode.StretchToFill, true);
			}
				...
				|| (dynamicJoystick && Application.isEditor && !Application.isPlaying))
			{
				...
				{
					GUI.color = new Color(areaColor.r,areaColor.g,areaColor.b,0.5f);	
					if (Application.isPlaying && !dynamicJoystick)
					{
						EasyTouch.instance.reservedVirtualAreas.Remove(areaRect);
					}
				...
				|| (dynamicJoystick && Application.isEditor && !Application.isPlaying))
			{
				...
				{
					GUI.color = new Color(touchColor.r,touchColor.g,touchColor.b,0.5f);	
				}		

				Rect drawTouchArea = new Rect(anchorPosition.x + joystickCenter.x + (joystickTouch.x - touchSize), 
					anchorPosition.y + joystickCenter.y - (joystickTouch.y + touchSize),
					touchSize * 2, 
					touchSize * 2);

                GUI.color = new Color(touchColor.r, touchColor.g, touchColor.b, 1);

                // 绘制点击区域;
                GUI.DrawTexture(drawTouchArea, touchTexture, ScaleMode.ScaleToFit, true);
			}	
		...
		else
		{
			if(!virtualJoystick)
			{
				...
				{
                    case DynamicArea.Left:
                        anchorPosition = new Vector2(zoneRadius, VirtualScreen.height - zoneRadius);
                        areaRect = new Rect(anchorPosition.x - zoneRadius, anchorPosition.y - zoneRadius, zoneRadius * 2, zoneRadius * 2);
                        break;
                    case DynamicArea.Right:
                        anchorPosition = new Vector2(VirtualScreen.width - zoneRadius, VirtualScreen.height - zoneRadius);
                        areaRect = new Rect(anchorPosition.x - zoneRadius, anchorPosition.y - zoneRadius, zoneRadius * 2, zoneRadius * 2);
                        break;
                    case DynamicArea.BottomLeft:
                        anchorPosition = new Vector2(zoneRadius, VirtualScreen.height - zoneRadius);
                    ...
                    case DynamicArea.BottomRight:
                        ...
                        break;
					case DynamicArea.Bottom:
						anchorPosition = new Vector2(VirtualScreen.width*0.5f, VirtualScreen.height*0.75f);
						areaRect = new Rect(anchorPosition.x - zoneRadius, anchorPosition.y - zoneRadius, zoneRadius * 2, zoneRadius * 2);
						break;
				}
				...
				// area zone;
				if ((showZone && areaTexture != null)
					|| (showZone && areaTexture != null) 
				...
				// area touch;
				if ((showTouch && touchTexture != null)
					|| (showTouch && touchTexture != null) 
					|| (Application.isEditor && !Application.isPlaying))
				{
					if (isActivated)
					{
                    ...
                    {
                        case DynamicArea.Left:
                            GUI.DrawTexture(new Rect(anchorPosition.x - touchSize,
                                anchorPosition.y - touchSize,
                                touchSize * 2, touchSize * 2),
                                touchTexture, ScaleMode.ScaleToFit,
                                true);
                            break;
                        case DynamicArea.Right:
                            GUI.DrawTexture(new Rect(anchorPosition.x - touchSize,
	...
	void CreateEvent(MessageName message, Vector2 pos,int iFingerID =0)
	{
		...
		move.joystick = this;
		move.postion = pos;
		move.bOutTouch = bOutTouch;
        move.iFingerID = iFingerID;
        ...
        if (!useBroadcast)
		{
			switch (message)
			{
				...
				case MessageName.On_JoystickTap:
					...
					{
                        Debug.Log("On_JoystickTap Click!!!!!!!!!!!!!!!!!!!!!!!!!!!");
						On_JoystickTap( move);	
					}
					break;				
	...
	void ComputeJoystickAnchor(JoystickAnchor anchor)
	{
        ...
        RectOperation = new Rect(anchorPosition.x + joystickCenter.x - zoneRadius, anchorPosition.y + joystickCenter.y - zoneRadius, zoneRadius * 2, zoneRadius * 2);
	...
	void On_TouchStart(Gesture gesture)
	{
		...
		//{
		//	Ray rayUI = UICamera.mainCamera.ScreenPointToRay(gesture.position);
		//	RaycastHit hitUI;
		//	if(Physics.Raycast(rayUI, out hitUI))
		//	{
		//		if(hitUI.transform != null 
		//			&& (hitUI.transform.name.Equals("Panel")
		//				|| hitUI.transform.name.Equals("PanelAttack")))
		//		{
		//			return;
		//		}

  //              Func func = UIRootCacheManager.GetInstance().Peek();
  //              if (func == null)
  //              {
  //                  return;
  //              }
  //              GameObject obj = func.mObj;
                
		//		if(obj != null && hitUI.transform != null && hitUI.transform.name.Equals(obj.name))
		//		{
		//			return;
		//		}
		//	}
			...
			if (isActivated)
			{
				if (!dynamicJoystick)
				{
					...
					if ((gesture.position - center).sqrMagnitude < (zoneRadius *VirtualScreen.xRatio )*(zoneRadius *VirtualScreen.xRatio ))
					{
						...
						CreateEvent(MessageName.On_JoystickTouchStart, gesture.position,gesture.fingerIndex);
					}			
				}
				else
				{
					if (!virtualJoystick)
					{
						...
						switch (area)
						{
							...
							// bottom
							case DynamicArea.Bottom:
								if (gesture.position.y< Screen.height/2 && gesture.position.y>100)
								{
									virtualJoystick = true;	
								}
								break;
	...
	void On_SimpleTap(Gesture gesture)
	{
		...
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick)
		{
			if (isActivated)
			{
				...
				{
					CreateEvent(MessageName.On_JoystickTap, gesture.position, gesture.fingerIndex);	
				}	
			}
		}
	}
	...
	void On_DoubleTap(Gesture gesture)
	{
		...
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick)
		{
			if (isActivated)
			{
				if (gesture.fingerIndex == joystickIndex)
				{
					CreateEvent(MessageName.On_JoystickDoubleTap, gesture.position, gesture.fingerIndex);	
				}
			}
		}
	}
	...
	{
		joystickPostion = gesture.position;
			...
			if (isActivated)
			{
				...
				if (gesture.fingerIndex == joystickIndex)
				{
					if (((gesture.position - center).sqrMagnitude < (zoneRadius *VirtualScreen.xRatio )*(zoneRadius *VirtualScreen.xRatio) && resetFingerExit) || !resetFingerExit)
					{
						...
						//Debug.Log((joystickTouch/(zoneRadius-touchSizeCoef)).sqrMagnitude);

						if((joystickTouch/(zoneRadius-touchSizeCoef)).sqrMagnitude >= 0.4f)
						{
							bOutTouch = true;
						}
						else
	...
	void On_TouchUp(Gesture gesture)
	{
		...
		if (gesture.fingerIndex == joystickIndex)
		{
			...
			CreateEvent(MessageName.On_JoystickTouchUp, gesture.position, gesture.fingerIndex);
		}
	}
	...
	public void On_Manual(Vector2 movement)
	{
		if (isActivated)
		{
			if (movement != Vector2.zero)
			{
				...
				{
					virtualJoystick = true;
					CreateEvent(MessageName.On_JoystickTouchStart, movement);
				}
			...
			else
			{
				if (virtualJoystick)
				{
					virtualJoystick = false;
					joystickIndex=-1;
					CreateEvent(MessageName.On_JoystickTouchUp, movement);
				}
			}
		}
	}
