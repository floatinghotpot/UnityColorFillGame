...
public class EasyTouch : MonoBehaviour {
	...
	void Update(){
		...
		if (enable && EasyTouch.instance==this){
            ...
            if (input.getTouchCount(true) > 0)
            {
                updateTouchImp(true);
            }
            else
            {
                updateTouchImp(false);
            }
        }
	}

    void updateTouchImp(bool bReal)
    {
        int i;

        // How many finger do we have ?
        int count = input.getTouchCount(bReal);
    ...
    public bool IsPointHoverNGui(Vector3 screenPoint)
    {
        bool returnValue = false;
        ...
        if (enabledNGuiMode)
        {
            ...
            while (!returnValue && i < nGUICameras.Count)
            {
                Ray ray = nGUICameras[i].ScreenPointToRay(screenPoint);
